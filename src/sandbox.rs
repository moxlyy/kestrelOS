...
/// Fetch a URL inside a sandbox that is otherwise just as isolated as a
/// normal build — except networking is deliberately left enabled. This is
/// the ONLY function in the whole tool that omits --unshare-net, and it
/// exists specifically so `[fetch]` derivations have a legitimate way to
/// touch the network without weakening the guarantee for `[build]`
/// derivations, which never get this option.
///
/// Safety comes from the caller: the result's hash gets checked against a
/// hash that was declared *before* the fetch ran (see main.rs). This
/// function only fetches; it does not decide whether to trust the result.
pub fn run_fetch(url: &str, out_path: &Path) -> Result<PathBuf> {
    let filename = url.rsplit('/').next().filter(|s| !s.is_empty()).unwrap_or("source");
    ...
    let status = cmd.status().context("spawning bwrap for fetch")?;
    if !status.success() {
        bail!("fetch failed: curl exited with {:?}", status.code());
    }
    Ok(dest)
}
